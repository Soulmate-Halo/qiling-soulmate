...
- 保留实时输出、准确退出码和硬超时兜底；仅真正超时才终止直接进程树，合法交还后的后台任务继续运行。
- 内容版本：内部 294；Soulmate 通道保持 287。

## v2.3.39 (2026-08-29)

- 内部桌面内容通道 293：停止后重新编辑消息会回填并保留原附件；附件路径不可恢复时在截断前提前拦截。
- 修复聊天主输入框在 Windows / Electron / IME 下焦点脱离：输入框重绑、主窗口回焦、编辑器与截图关闭后回焦；悬浮球不再抢焦。
- 内容版本：内部 293；Soulmate 通道保持 287。

## v2.3.38 (2026-08-28)

- 内部桌面内容通道 292：聚合调度由 ask-only 改为同会话渠道与模型的可调用工具 agent，并注入项目线索摘要；agent 失败回落 ask。
- 器灵僵尸收口加固：真杀仅限 stalled，且同时满足 6 小时硬顶、连续静默 30 分钟、PID 启动时间与命令行匹配、杀后确认。
- 顶栏定时按钮改为打开定时任务面板，不再误跳到提醒会话。
- 内容版本：内部 292；Soulmate 通道保持 287。
